...
export class NotesService {
    ...
    getNotes(): Observable<Note[]> {
        return this._httpClient.get<any>(this.getApiUrl('notes')).pipe(
    ...
    createNote(note: Note): Observable<Note> {
        return this._httpClient
            .post<any>(this.getApiUrl('notes'), note)
            .pipe(
                map(response => environment.useRealApi ? response.data : response),
                switchMap((response) =>
                    this.getNotes().pipe(
    ...
    updateNote(note: Note): Observable<Note> {
        ...
        if (updatedNote.labels && updatedNote.labels.length) {
            updatedNote.labels = updatedNote.labels.map((label) => label.id || label);
        }

        return this._httpClient
            .put<any>(this.getApiUrl(`notes/${note.id}`), updatedNote)
            .pipe(
                map(response => environment.useRealApi ? response.data : response),
                tap((response) => {
                    // Update the notes
    ...
    deleteNote(note: Note): Observable<boolean> {
        return this._httpClient
            .delete<any>(this.getApiUrl(`notes/${note.id}`))
            .pipe(
                map((response) => {
